...
function validateNoticiaBody(body, allowed = []) {
  const nombre = sanitizeTextField(body.nombre || body.titulo || '');
  const descripcion = sanitizeTextField(body.descripcion || '');
  const info = sanitizeTextField(body.info || body.contenido || '', { allowEmpty: true }) || null;
  const fecha = body.fecha || null;
  const direccion = sanitizeTextField(body.direccion || body.lugar || '', { allowEmpty: true }) || null;
...
async function createNoticia(req, res) {
  const allowed = await getAllowedFiltros('noticia');
  const parsed = validateNoticiaBody(req.body, allowed);
  if (parsed.error) {
...
async function updateNoticia(req, res) {
  ...
  try {
    ...
    await logMovimiento(req, {
      ...
      detalle: `Editó noticia «${parsed.nombre}»`,
    });
  } catch (err) {
...
async function listPublicNoticias(req, res) {
  ...
  try {
    ...
    const [rows] = await pool.execute(
      `${LIST_SELECT} WHERE 1=1${clause} ORDER BY COALESCE(n.creado_en, CAST(n.fecha AS DATETIME)) DESC, n.id_noticia DESC LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
...
async function deleteNoticia(req, res) {
  ...
  try {
    const [existing] = await pool.execute(
      'SELECT id_usuario, img_url, nombre FROM noticias WHERE id_noticia = ? LIMIT 1',
      [id]
    );
    if (!existing.length) {
      return res.status(404).json({ ok: false, message: 'Noticia no encontrada' });
    ...
    await logMovimiento(req, {
      ...
      detalle: `Eliminó noticia «${existing[0].nombre}»`,
    });
  } catch (err) {
