...
const LIST_SELECT = `
  SELECT q.id_lugar, q.nombre, q.descripcion, q.info, q.contacto, q.img_url,
...
function validateQueVisitarBody(body) {
  const nombre = (body.nombre || '').trim();
  const descripcion = (body.descripcion || '').trim();
  const info = (body.info || body.informacion || '').trim() || null;
  const contacto = (body.contacto || '').trim() || null;

  if (!nombre) return { error: 'El nombre es obligatorio' };
  if (!descripcion) return { error: 'La descripción es obligatoria' };

  return { nombre, descripcion, info, contacto };
}

// POST nuevo lugar con al menos una imagen.
async function createQueVisitar(req, res) {
  const parsed = validateQueVisitarBody(req.body);
  if (parsed.error) {
  ...
  try {
    const outcome = await insertThenSaveImages({
      ...
      files: req.files,
      insertSql: `INSERT INTO que_visitar (nombre, descripcion, info, contacto, id_usuario, img_url)
        VALUES (?, ?, ?, ?, ?, NULL)`,
      insertParams: [
        parsed.nombre,
        parsed.descripcion,
...
async function updateQueVisitar(req, res) {
  ...
  try {
    ...
    await pool.execute(
      `UPDATE que_visitar SET nombre = ?, descripcion = ?, info = ?, contacto = ?, img_url = ? WHERE id_lugar = ?`,
      [parsed.nombre, parsed.descripcion, parsed.info, parsed.contacto, imgResult.serialized, id]
    );
...
async function listPublicQueVisitar(req, res) {
  ...
  try {
    const [[{ total }]] = await pool.execute('SELECT COUNT(*) AS total FROM que_visitar');
    const [rows] = await pool.execute(
      `${LIST_SELECT} ORDER BY q.id_lugar DESC LIMIT ? OFFSET ?`,
      [limit, offset]
    );
